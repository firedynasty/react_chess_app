#!/usr/bin/env python3
"""
Fork of report.py for multi-source buckets (chess.com + Lichess): reads
games from every raw/<source>/ dir instead of hardcoding raw/chesscom, so
Lichess-only or mixed buckets (like data/firebirdbot) actually find games.

Prototype: AI game report for a group of games from the same opening position.

Usage (from inside a bucket folder, data/<key>/):
    python lichess_report.py <game_id> [game_id ...]
    python lichess_report.py 164380092682 171996841806 172074667356
    python lichess_report.py --position "d4 d5 e3 Nc6 Bd3 Nf6 f4"

Usage (from chess-rag/ — point at a bucket with --key):
    python lichess_report.py --key firebirdbot --position "d4 d5"
...
Quantitative stats export (no OpenAI key needed, runs over every game in the bucket):
    python lichess_report.py --key firebirdbot --stats-csv
    python lichess_report.py --key firebirdbot --stats-csv --blunder-cp 150
...
RAW_DIRS = [HERE / "data" / "raw" / "chesscom"]
...
def resolve_bucket(key: str | None) -> Path:
    ...
    if HERE.parent.name == "data":
        return HERE
    raise SystemExit(
        "Pass --key <name> (e.g. python lichess_report.py --key firebirdbot …) "
...
def find_pgn_path(game_id: str) -> Path | None:
...
def load_pgn(game_id: str) -> str | None:
    path = find_pgn_path(game_id)
    if path is None:
        print(f"[warn] PGN not found for {game_id} in {RAW_DIRS}")
        return None
    return path.read_text(encoding="utf-8")
...
def run_stats_export(bucket: Path, blunder_cp: int = 100) -> None:
    ...
    raw_root = bucket / "raw"
    raw_dirs = sorted(p for p in raw_root.iterdir() if p.is_dir()) if raw_root.exists() else []
    pgn_files = sorted(f for raw_dir in raw_dirs for f in raw_dir.glob("*.pgn"))
    if not pgn_files:
        sys.exit(f"No PGNs found under {raw_root}/*")

    print(f"Analyzing {len(pgn_files)} games in {raw_root}/* for phase/time/blunder stats...")
...
def main() -> None:
    global RAW_DIRS, DB_PATH, REPORTS_DIR
