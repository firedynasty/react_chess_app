#!/usr/bin/env python3
"""
Generate tree_viewer.html for a multi-source bucket (chess.com + Lichess).

Fork of tree_viz.py: identical except it reads every raw/<source>/*.pgn
directory under the bucket (raw/chesscom AND raw/lichess) instead of
hardcoding raw/chesscom, so buckets that mix both sources (or are
Lichess-only, like data/firebirdbot) get every game embedded.

Reads <bucket>/tree.sqlite + <bucket>/raw/*/*.pgn, embeds everything as
...
Or from inside a bucket folder — no arguments needed:
    python lichess_tree_viz.py

Overrides:
    --db PATH  --pgn-dir PATH  --out PATH
    (--pgn-dir still accepts a single directory, same as tree_viz.py)
"""
...
def load_pgns(pgn_dirs: Path | list[Path], eco_lookup: dict | None = None) -> dict:
    """
    pgn_dirs: a single directory, or a list of them (e.g. raw/chesscom AND
    raw/lichess) — every *.pgn file found across them is loaded, keyed by
    filename stem (game_id).
...
def main() -> None:
    ...
    parser.add_argument("--pgn-dir", default=None, help="default: every <bucket>/raw/* dir (chesscom, lichess, ...)")
