#!/usr/bin/env python3
"""
Fork of tree_engine.py, kept alongside the Lichess pipeline (lichess_ingest.py,
lichess_tree_viz.py, lichess_report.py) so it never gets edited-and-forgotten
out of sync. Functionally identical to tree_engine.py: it builds from the
bucket's merged games.pgn, which is already source-agnostic (multi-source
merge lives in ingest.py's rebuild_games_pgn), so no chesscom-specific paths
...
Build the tree (from chess-rag/, using a bucket key):
    python lichess_tree_engine.py firebirdbot

Or from inside a bucket folder (data/<key>/) — no arguments needed:
    python lichess_tree_engine.py

Query the tree:
    python lichess_tree_engine.py firebirdbot --query e4 e5 Nf3
